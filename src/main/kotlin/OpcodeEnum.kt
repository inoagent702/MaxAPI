enum class OpcodeEnum(val opcode: Short) {
    PING(1),
    DEBUG(2),
    RECONNECT(3),
    LOG(5),
    ...
    companion object {
        private val opcodeNameCache: MutableMap<Short, String> = HashMap()

        private val opcodeToEnum: Map<Short, OpcodeEnum> = entries.associateBy { it.opcode }

        init {
            for (entry in entries) {
                opcodeNameCache[entry.opcode] = entry.name
            }
        }

        @JvmStatic
        fun nameOf(opcode: Short): String {
            opcodeNameCache[opcode]?.let { return it }

            val hex = (opcode.toInt() and 0xFFFF).toString(16)
            val formatted = "0x$hex"
            opcodeNameCache[opcode] = formatted
            return formatted
        }

        @JvmStatic
        fun fromOpcode(opcode: Short): OpcodeEnum? = opcodeToEnum[opcode]
    }
}
